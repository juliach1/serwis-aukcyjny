                            ...
                            <c:forEach var ="messageDTO" items = "${messageDTOS}" varStatus="index">
                                <c:choose>
                                    <c:when test="${index.index>0 && messageDTO.sendTime.toLocalDate() != messageDTOS.get(index.index-1).sendTime.toLocalDate()}">
                                        <div class="divider d-flex justify-content-center mb-4 text-muted">
                            ...
                            <c:choose>
                                <c:when test="${principalDTO.id == messageDTO.sender.id}">
                                    <div class="d-flex flex-row justify-content-end">
                                        <div class="me-3">
                                            <p class="small p-3 ms-3 mb-1 bg-light-purple text-light rounded-3">${messageDTO.content}</p>
                                            <p class="small ms-3 mb-3 rounded-3 text-muted">${messageDTO.sendTime.toLocalTime()}</p>
                                        </div>
                                        <c:choose>
...
<%--                                             alt="avatar 1" style="width: 45px; height: 100%;">--%>
                                        <div class="ms-3">
                                            <p class="small p-3 mb-1 rounded-3" style="background-color: #f5f6f7;">${messageDTO.content}</p>
