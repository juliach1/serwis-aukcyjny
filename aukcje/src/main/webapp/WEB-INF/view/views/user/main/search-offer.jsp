...
<head>
    ...
    <title>Szukaj oferty - SellBuy</title>
...
<%--       ${offerSearchModel.offerTypes.forEach(checkboxChecker)};--%>

<%--       function checkboxChecker(model){--%>
<%--           if(model === "kup teraz"){--%>
...
<main>
    ...
    <section class="central container col-sm-12 col-md-10 col-xl-8">
        ...
        <section id="main-search-info" class="bg-light p-5 mt-5">
            ...
            <section id="offers mt-4">

                <jsp:useBean id="offers" scope="request" type="java.util.List<com.aukcje.dto.OfferDTO>"/>
                <div class="result-offers row cols-md-2 cols-lg-4 cols-xl-6 px-3">
                    <p class="display-5 text-left mt-5">
                        Wyniki wyszukiwania dla "<span>${offerSearchModel.phrase}</span>"...
                    </p>

                    <c:if test="${offers.size() < 1}">
                        <div class="noresultbox p-5 my-5 border">
                            <p class=" display-8 text-danger"> Nie znaleziono pasujących wyników... Spróbuj użyć innych słów kluczowych lub modyfikować filtry.</p>
                        </div>
                    </c:if>
...
<script src="${pageContext.request.contextPath}/files/js/user/main/search-offer.js"></script>
